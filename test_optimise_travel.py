...
def test_all_indices_present():
    coordinates = [(0, i/1000) for i in range(314)]
...
def test_case_1():
    ...
    total_distance = optimise_travel.calc_total_distance(
        coordinates_xy, indices)
    assert len(indices) == len(coordinates)
    assert total_distance <= 45000
